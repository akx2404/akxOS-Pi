...
POLL_S = 0.5
WORKLOAD_CMD = ["python3", "tests/fixed_workload.py"]
...
def collect(pid: int, budget_mw: int, duration_s: float):
    ...
    while True:
        ...
        if row:
            ...
            print(
                f"t={t:5.1f}s | waiting for PID {pid} in /proc...",
                flush=True,
            )
...
def recommend_window(metrics, budget, tol_pct):
    tol = budget * tol_pct / 100.0
    ...
    for w in sorted(metrics):
        ...
        if m["settle_s"] is not None and m["ss_sigma"] <= tol:
            print(
                f"Use window={w} "
                f"({w * POLL_S:.1f}s smoothing): "
                f"settle={m['settle_s']:.1f}s, "
                f"SS σ={m['ss_sigma']:.2f} mW"
            )
            return

    best_w = min(metrics, key=lambda x: metrics[x]["ss_sigma"])
    m = metrics[best_w]

    print(
        f"No window fully satisfies ±{tol:.1f} mW stability.\n"
        f"Best available: window={best_w}, "
        f"SS σ={m['ss_sigma']:.2f} mW, "
        f"SS error={m['ss_error']:.2f} mW"
    )
...
def plot_results(data, metrics, tol_pct, out_path):
    times = data["times"]
    raw = data["raw_power"]
    budget = data["budget_mw"]
    tol = budget * tol_pct / 100.0
    ...
    for w in sorted(metrics.keys()):
        ...
        label = f"w={w} ({w * POLL_S:.1f}s), settle="
        label += "never" if st is None else f"{st:.1f}s"
    ...
    ax_power.fill_between(
        times,
        [budget - tol] * len(times),
        [budget + tol] * len(times),
        alpha=0.12,
        label=f"±{tol_pct}% band",
    )
    ...
    ax_quota.set_title("Controller action: quota and stop window")
...
def run_one(pid, budget, duration, windows, tol):
    ...
    if len(data["times"]) < 4:
        print("Too few samples. Workload probably exited early.")
        return None, None

    metrics = analyse(data, windows, tol)

    print_table(metrics, budget, tol)
    recommend_window(metrics, budget, tol)

    stem = f"settling_pid{pid}_budget{budget}mw"
    png_path = OUTPUT_DIR / f"{stem}.png"
    csv_path = OUTPUT_DIR / f"{stem}.csv"

    plot_results(data, metrics, tol, png_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="akxOS scheduler budget settling and oscillation experiment"
    )
    ...
    try:
        for i, budget in enumerate(budgets):
            print("\n" + "─" * 60)
            print(f"Budget = {budget} mW | PID = {pid} | Duration = {args.duration}s")
            print("─" * 60)
            ...
            if i != len(budgets) - 1:
                print("\nCooldown 10s before next budget...")
                time.sleep(10)
